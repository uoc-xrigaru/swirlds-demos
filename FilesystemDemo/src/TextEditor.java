...
/** A simple text editor that saves files on a Swirlds Fast Copyable Filesystem.
 *  Every save is transmitted as a transaction to peer nodes.
 *  This code also demonstrates how to invoke the graphical FileExplorer to
 *  load or save files in a filesystem.
 */
public class TextEditor extends JPanel {
  ...
  private void saveAs(ActionEvent e) {
    try {
      ...
      boolean done = false;
      while(!done) {
        String pathname = FileExplorer.choose(topFrame, state.getFS(), true);
        if (pathname == null)
          done = true;
        else if (state.getFS().isDir(pathname))
          showMessage(String.format("Sorry, \"%s\" is a directory.", pathname));
        else {
          done = true;
